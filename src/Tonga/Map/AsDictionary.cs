...
namespace Tonga.Map
{
    ...
    public sealed class AsDictionary<Key, Value> : IDictionary<Key, Value>
    {
        ...
        public void Add(Key key, Value value) => this.memory.Value.Add(key, value);
        public void Add(KeyValuePair<Key, Value> item) => this.memory.Value.Add(item);
        public void Clear() => this.memory.Value.Clear();
        public bool Contains(KeyValuePair<Key, Value> item) =>
            this.memory.Value.Contains(item);
        public bool ContainsKey(Key key) => this.memory.Value.ContainsKey(key);
        public void CopyTo(KeyValuePair<Key, Value>[] array, int arrayIndex) =>
            memory.Value.CopyTo(array, arrayIndex);
        public IEnumerator<KeyValuePair<Key, Value>> GetEnumerator() =>
            this.memory.Value.GetEnumerator();
        public bool Remove(Key key) => memory.Value.Remove(key);
        public bool Remove(KeyValuePair<Key, Value> item) => this.memory.Value.Remove(item);
        public bool TryGetValue(Key key, [MaybeNullWhen(false)] out Value value) =>
            this.memory.Value.TryGetValue(key, out value);
        IEnumerator IEnumerable.GetEnumerator() => this.memory.Value.GetEnumerator();
    }
