...
/// </summary>
public sealed class EndsWith(TextMorph text, TextMorph tail) : FactEnvelope(
...
)
{
    ...
    public EndsWith(IText text, IText tail) : this(
        new TextMorph(text),
        new TextMorph(tail)
    )
    { }
    ...
    /// <param name="text">Text to test</param>
    /// <param name="tail">Ending content to use in the test</param>
    public EndsWith(IText text, TextMorph tail) : this(
        new TextMorph(text),
        tail
    )
    { }
