...
namespace Tonga.Map
{
    ...
    public sealed class Fallback<Key, Value> : IMap<Key, Value>
    {
        ...
        public Fallback(IMap<Key, Value> map, IMap<Key, Value> fallbackMap) : this(
            () => map, fallbackMap
                        )
        { }
        ...
        public Fallback(IMap<Key, Value> map, Func<Key, Value> fallback) : this(
            () => map, fallback
        )
        { }
        ...
        /// <param name="map">Map returning existing values</param>
        /// <param name="fallback">Fallback generating missing values</param>
        public Fallback(Func<IMap<Key, Value>> map, Func<Key, Value> fallback)
        {
            this.origin = map;
    ...
    public static class Fallback
    {
        ...
        public static IMap<Key, Value> _<Key, Value>(IMap<Key, Value> map, IMap<Key, Value> fallbackMap)
            => new Fallback<Key, Value>(map, fallbackMap);
        ...
        public static IMap<Key, Value> _<Key, Value>(IMap<Key, Value> map, Func<Key, Value> fallback)
            => new Fallback<Key, Value>(map, fallback);
