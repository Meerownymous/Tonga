...
public static partial class EnumerableSmarts
{
    ...
    /// A <see cref="IEnumerable{T}"/> with the given items sorted by default.
    /// </summary>
    public static IEnumerable<T> AsSorted<T>(this T[] src) where T : IComparable<T> =>
        new Sorted<T>(src);

    /// <summary>
    /// A <see cref="IEnumerable{T}"/> sorted by the given <see cref="Comparer{T}"/>.
    /// </summary>
    public static IEnumerable<T> AsSorted<T>(this IEnumerable<T> src) where T : IComparable<T> =>
        new Sorted<T>(src);

    /// <summary>
    /// A <see cref="IEnumerable{T}"/> sorted by the given <see cref="Comparer{T}"/>.
    /// </summary>
    public static IEnumerable<T> AsSorted<T>(this IEnumerable<T> src, Comparer<T> cmp) =>
