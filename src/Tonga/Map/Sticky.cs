...
namespace Tonga.Map
{
    ...
    /// </summary>
    public sealed class Sticky<Key, Value>(Func<IMap<Key, Value>> source) : IMap<Key, Value>
    {
    ...
    public static partial class MapSmarts
    {
        ...
        /// A map whose keys and values are sticky per value once retrieved for the first time time.
        /// </summary>
        public static IMap<Key,Value> AsSticky<Key, Value>(this IMap<Key, Value> origin) =>
            new Sticky<Key, Value>(origin);

        /// <summary>
        /// A map whose keys and values are sticky per value once retrieved for the first time time.
        /// </summary>
        public static IMap<Key, Value> AsSticky<Key, Value>(this Func<IMap<Key, Value>> origin) =>
            new Sticky<Key, Value>(origin);
    }
}
